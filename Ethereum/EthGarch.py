...
ETH_Prices = pdr.DataReader('ETH-USD', start, end)

# Calculate Returns as a percentage change
ETH_Prices['Return'] = 100 * (ETH_Prices['Close'].pct_change())
# Calculate Log returns as a number
ETH_Prices['Log_Return'] = np.log(ETH_Prices['Close']) - np.log(ETH_Prices['Close'].shift(1))
# Drops rows that have incomplete/NULL data
ETH_Prices = ETH_Prices.dropna()

# Calculate the volatility of log returns
ETH_Daily_Vol = ETH_Prices['Log_Return'].std()
# Round the volatility to 2 decimal places
ETH_Daily_Vol = round(ETH_Daily_Vol, 2)
# Calculate the volatility of normal returns
ETH_Daily_Vol_Normal = ETH_Prices['Return'].std()
# Round the volatility to 2 decimal places
ETH_Daily_Vol_Normal = round(ETH_Daily_Vol_Normal, 2)
# Basic Garch (1, 1) model created
basic_gm = arch_model(ETH_Prices['Log_Return'], p = 1, q = 1,
...
plt.plot(ETH_Prices['Log_Return'].loc[ind_list].sub(ETH_Prices['Log_Return'].loc[ind_list].mean()).pow(2),
...
plt.title('Ethereum GARCH Volatility Forecasting vs Actual')
#plt.yscale('linear')
...
def evaluate(observation, forecast):
    ...
    print(f'Root Mean Squared Error (RMSE): {round(math.sqrt(mse), 3)}')
    return mae, mape, math.sqrt(mse)

# Backtest model with MAE, MAPE and RMSE
evaluate(ETH_Prices['Log_Return'].loc[ind_list].sub(ETH_Prices['Log_Return'].loc[ind_list].mean()).pow(2), rolling_volatility ** 2)
