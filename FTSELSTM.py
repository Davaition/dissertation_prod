...
FTSE_Prices = pd.read_csv('FTSE.csv')
FTSE_Prices = FTSE_Prices[['Date', 'Close']]
...
FTSE_Prices['Date'] = FTSE_Prices['Date'].apply(str_to_datetime)
...
windowed_df = df_to_windowed_df(FTSE_Prices,
                                '2018-10-12',
                                '2023-06-08',
                                n=3)
...
model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=598)
...
plt.title('FTSE LSTM Training Forecasted vs Actual Close Values')
...
plt.title('FTSE LSTM Validation Forecasted vs Actual Close Values')
...
plt.title('FTSE LSTM Testing Forecasted vs Actual Close Values')
...
#al_Convert_Close_Prices['Val_Return'] = np.log(Val_Convert_Close_Prices['Close']) - np.log(Val_Convert_Close_Prices['Close'].shift(1))
...
plt.plot(Train_Convert_Close_Prices['Train_Return'], color= 'red', label= 'Forecasted FTSE LSTM Training Data Returns')
plt.plot(Val_Convert_Close_Prices['Val_Return'], color= 'black', label= 'Forecasted FTSE LSTM Validation Data Returns')
plt.plot(Test_Convert_Close_Prices['Test_Return'], color= 'blue', label= 'Forecasted FTSE LSTM Testing Data Returns')
plt.title('FTSE LSTM Forecasted Returns')
plt.legend(loc = 'lower left')
plt.show()

Train_Convert_Close_Prices['Date'] = Train_Convert_Close_Prices.index
Train_Convert_Close_Prices.to_csv('FTSE Train Data.csv')
Val_Convert_Close_Prices['Date'] = Val_Convert_Close_Prices.index
Val_Convert_Close_Prices.to_csv('FTSE Val Data.csv')
Test_Convert_Close_Prices['Date'] = Test_Convert_Close_Prices.index
Test_Convert_Close_Prices.to_csv('FTSE Test Data.csv')

Train_Convert_Close_Prices = pd.read_csv('FTSE Train Data.csv')
Train_Convert_Close_Prices = Train_Convert_Close_Prices[['Date', 'Close']]
Train_Convert_Close_Prices = Train_Convert_Close_Prices.dropna()

Val_Convert_Close_Prices = pd.read_csv('FTSE Val Data.csv')
Val_Convert_Close_Prices = Val_Convert_Close_Prices[['Date', 'Close']]
Val_Convert_Close_Prices = Val_Convert_Close_Prices.dropna()

Test_Convert_Close_Prices = pd.read_csv('FTSE Test Data.csv')
