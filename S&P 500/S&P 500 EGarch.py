...
basic_gm = arch_model(GSPC_Prices['Log_Return'], p = 1, q = 1, o = 2,
...
plt.plot(GSPC_Prices['Log_Return'].loc[ind_list].sub(GSPC_Prices['Log_Return'].loc[ind_list].mean()).pow(2),
         color = 'black', alpha = 0.4, label = 'Actual Daily Volatility')

# Plot EGARCH  estimated volatility
plt.plot(rolling_volatility**2, color = 'red', label = 'Forecasted EGARCH Volatility')
plt.title('S&P 500 EGARCH Volatility Forecasting vs Actual')
#plt.yscale('linear')
