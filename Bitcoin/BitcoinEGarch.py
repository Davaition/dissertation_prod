...
Bitcoin_Prices = pdr.DataReader('BTC-USD', start, end)

# Calculate Returns as a percentage change
Bitcoin_Prices['Return'] = 100 * (Bitcoin_Prices['Close'].pct_change())
# Calculate Log returns as a number
Bitcoin_Prices['Log_Return'] = np.log(Bitcoin_Prices['Close']) - np.log(Bitcoin_Prices['Close'].shift(1))
# Drops rows that have incomplete/NULL data
Bitcoin_Prices = Bitcoin_Prices.dropna()

# Calculate the volatility of log returns
Bitcoin_Daily_Vol = Bitcoin_Prices['Log_Return'].std()
# Round the volatility to 2 decimal places
Bitcoin_Daily_Vol = round(Bitcoin_Daily_Vol, 2)
# Calculate the volatility of normal returns
Bitcoin_Daily_Vol_Normal = Bitcoin_Prices['Return'].std()
# Round the volatility to 2 decimal places
Bitcoin_Daily_Vol_Normal = round(Bitcoin_Daily_Vol_Normal, 2)
# Basic Garch (1, 1) model created
basic_gm = arch_model(Bitcoin_Prices['Log_Return'], p = 1, q = 1, o = 2,
...
plt.plot(Bitcoin_Prices['Log_Return'].loc[ind_list].sub(Bitcoin_Prices['Log_Return'].loc[ind_list].mean()).pow(2),
...
plt.title('Bitcoin EGARCH Volatility Forecasting vs Actual')
#plt.yscale('linear')
...
evaluate(Bitcoin_Prices['Log_Return'].loc[ind_list].sub(Bitcoin_Prices['Log_Return'].loc[ind_list].mean()).pow(2), rolling_volatility**2)
